...
class Library:
    ...
    def return_book(self, book_number):
        ...
        query = "UPDATE Books SET Borrowed = 0, Deadline = NULL WHERE Number = ?"
        self.cursor.execute(query, (book_number,))
        self.connection.commit()
...
app = FastAPI()
templates = Jinja2Templates(directory="C:\\Users\\ENES CATAL\\Desktop\\PPYProject-arzununki\\PPYProject-arzununki\\templates")
...
async def query_book(request: Request):
    try:
        ...
        if books:
            return templates.TemplateResponse("queryBook.html", {"request": request, "books": books})
        else:
            return templates.TemplateResponse("queryBook.html", {"request": request, "message": "No such book exists."})
    except Exception as e:
        traceback.print_exc()
        raise HTTPException(status_code=500, detail="Failed to query book!")
...
async def show_update_book_edition_form(request: Request):
    return templates.TemplateResponse("update_book_edition.html", {"request": request})

@app.post("/update-book-edition")
async def update_book_edition(request: Request, book_name: str = Form(...), new_edition: int = Form(...)):
    if not library.logged_in:
        raise HTTPException(status_code=400, detail="You are not logged in.")
    ...
    return templates.TemplateResponse("update_book_edition.html", {"request": request, "book_name": book_name})
...
#return book
@app.get("/return-book", response_class=HTMLResponse)
def show_return_book_form(request: Request):
    return templates.TemplateResponse("returnBook.html", {"request": request})

@app.post("/return-book")
async def return_book(book_number: int = Form(...)):
    library = Library()
    result = library.return_book(book_number)
    library.finish_connection()

    if result:
        return {"message": f"Book with number {book_number} has been returned successfully"}
    else:
        return {"message": f"Book with number {book_number} is not currently borrowed"}
